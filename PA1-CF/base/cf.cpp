 ...
 *  
 * Copyright © 2009-2013 Behnam Momeni.
 * 
...
void ClientFramework::usage () const {
  ...
  std::cout
      ...
      << std::endl;
  std::cout
      << "   \\ \\_\\ \\__/.\\_\\\\ \\_\\  \\ \\__\\ \\____/\\ \\___/     \\ \\____/ \\ \\_\\ "
      << std::endl;
  std::cout << "    \\/_/\\/__/\\/_/ \\/_/   \\/__/\\/___/  \\/__/       \\/___/   \\/_/ "
      << std::endl;
  std::cout << "                                                                "
      << std::endl;
  std::cout << "Partov Project - Version 2.5 -- Client Framework" << std::endl;
  std::cout << "Code-name: PARTOV (Portable And Reliable Tool fOr Virtualization)"
      << std::endl;
  std::cout << "Copyright © 2009-2013 Behnam Momeni." << std::endl;
  std::cout << std::endl;
  ...
  std::cout
      << "       --ip <server-ipv4>      The ip (version 4) of the Partov server (like 192.168.0.1)"
      << std::endl;
  std::cout << "       --port <server-port>    The port of the Partov server (like 9339)"
      << std::endl;
  std::cout
      << "       --map <map-name>        The name of the map/topology which you want to connect to (like router)"
      << std::endl;
  std::cout
      << "       --user <user-name>      Your username; will be used for authentication."
      << std::endl;
  std::cout
      << "       --pass <password>       Your password; will be used for authentication."
      << std::endl;
  std::cout
      << "       --node <node-name>      (optional) The name of the node which you want to simulate"
      << std::endl;
  std::cout << "                                    it within <map-name> map (if any)"
      << std::endl;
  std::cout
      << "       --id <creator-username> (not used in conjunction with --new or --free options)"
      << std::endl;
  std::cout
      << "                                    Try to connect to the map instance which was created"
      << std::endl;
  std::cout
      << "                                    by <creator-username> user previously."
      << std::endl;
  std::cout
      << "       --new                   (not used in conjunction with --id or --free options)"
      << std::endl;
  std::cout << "                                    Try to create a new map instance."
      << std::endl;
  std::cout
      << "                                    Any user can only create (at maximum) ipu instance(s) of each map."
      << std::endl;
  std::cout
      << "       --free                  (not used in conjunction with --id or --new options)"
      << std::endl;
  std::cout
      << "                                    Free resources of the map instance which was created"
      << std::endl;
  std::cout
      << "                                    by this user. Any user can only remove his/her owned instances."
      << std::endl;
  std::cout
      << "       --args <arg>[ <arg>...] (optional) The arguments which come after --args will be passed to"
      << std::endl;
  std::cout
      << "                                    the SimulatedMachine::parseArguments method. The --args must be"
      << std::endl;
  std::cout
      << "                                    latest argument which is not intended to be passed to user program."
      << std::endl;
